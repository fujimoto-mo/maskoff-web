...
- `IntroVeil`（client）— 参考サイトの `.intro-veil` の手順をそのまま再現する（2026-09-01 に実測: logo-in ≈ 0.45s → clip-path 0.75s → done で opacity 0.12s）:
  ...
  3. `data-logo-in` から 450ms（≤640 は 780ms）後、黒幕本体 `.veil-curtain`（SSR では画面中央の 300vmax の角丸 22% 正方形。収縮直前にロゴ箱の中心へ置き直し、一辺 = 最遠コーナー距離 × 2.4）を WAAPI で `transform: translate(-50%,-50%) scale(1 → ロゴ箱幅 / 一辺)` へ 0.75s `cubic-bezier(.65,0,.35,1)` で縮める（黒がロゴに集まり、そのままロゴセルの黒い箱になる。角丸 22% は scale で箱の角丸と一致）。**`clip-path` ではなく transform を使う**: clip-path はメインスレッド駆動で、ハイドレーション直後の処理や画像デコードと重なると途中で止まる（dev で 83ms の停止を実測）。transform はコンポジタで動くため詰まらない。
