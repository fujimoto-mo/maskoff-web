...
> **実装後の確定仕様（コントローラ修正 — 本文のコードより優先）:** 幕の終了は「フェード + ロゴ縮小」ではなく **FLIP でロゴをマーキーのロゴセルへ飛ばす**。`.veil-logo` はロゴセルと同構成の箱（`bg-fg` 角丸 + `p-[12%]` + ロゴ、PC 340px / SP 260px、`flex-none`）。`done` で `.veil-logo` と `[data-lead] [data-lead-box]` の rect を計測し、`translate(dx,dy) scale(目標幅/現在幅)` を WAAPI 0.6s `cubic-bezier(.22,1,.36,1)` で適用、背景は `.intro-veil[data-phase="done"] { background-color: transparent; transition: .4s }`（背景色は utilities の `bg-bg-dark` ではなく components 層の `.intro-veil` に書く）。着地で `data-boing` / `data-intro` 除去 / `kv:launch` / アンマウント。`Marquee.tsx` のロゴ箱に `data-lead-box`、`[data-marquee][data-js] [data-lead]` は `opacity: 0`（`data-go` で表示）。`MarqueeDrag` は `launched` フラグで `kv:launch` まで rAF を回さない。マーキーの CSS アニメーションは utilities の `animate-[drift…]` から components 層の `.mq-track { animation: drift … }` / `[data-reverse] .mq-track { animation-name: drift-rev }` へ移す（utilities だと `[data-js] .mq-track { animation:none }` が負ける）。実装は `src/components/motion/IntroVeil.tsx` を正とする。
