...
/** 静的ページは lastModified を出さない（ビルド日を毎回入れると更新の合図にならない）。NEWS / NOTICE は公開日 */
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  return [
    ...
    ...news.map((n) => ({ url: `${SITE.url}/news/${encodeURIComponent(n.slug)}/`, lastModified: new Date(n.publishedDate), changeFrequency: "monthly" as const, priority: 0.5 })),
    { url: `${SITE.url}/notice/`, changeFrequency: "weekly", priority: 0.5 },
    ...notices.map((n) => ({ url: `${SITE.url}/notice/${encodeURIComponent(n.slug)}/`, lastModified: new Date(n.publishedDate), changeFrequency: "monthly" as const, priority: 0.4 })),
