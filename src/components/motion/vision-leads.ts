 ...
 * @example pointsAttr(LEADS_SP[0]) → "250,95 290,55 297,55"
 */
export const VIEW = { w: 540, h: 440 } as const;

/**
 * SP（≤600px）の配置。すべて幅（540 = 100%）に対する比率で、VisionDiagram が CSS 変数として渡す。
 * - hrLeft: HR ブロックの左端。ブロックは w = 1 - hrLeft = 45% で右寄せ（self-end）。360px 幅でも「人材・採用支援事業」+ 矢印が 1 行に入る幅
 * - hrBottom: HR ブロックの下端 = SVG 上端から幅の 11% 下（margin-bottom: -11% で SVG に重ねる）
 * - rowTop: MK / CR の下段の上端 = SVG 下端から幅の 12% 上（margin-top: -12%）
 * - col: 下段の各列の幅（2 列で 98%。中央 2% の隙間に立方体の下の頂点が来る）
 */
export const SP = { hrLeft: 0.55, hrBottom: 0.11, rowTop: 0.12, col: 0.49 } as const;
 ...
 * MK / CR は左面・右面から斜め下へ → 垂直で下段の上端（y = 440 − 540 × rowTop）へ。
 */
export const LEADS_SP: readonly (readonly Pt[])[] = [
  [p(250, 95), p(290, 55), p(VIEW.w * SP.hrLeft, 55)],
