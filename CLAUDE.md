...
暗転時（VISIONの`data-on-vision`のマーカー帯）は半透明の `color-mix(in srgb, var(--color-marker) 60%, transparent)` を黒地に重ね、対象文字は前後とも本文と同じ白のまま（色は変えない — 参考サイトと同じ）。CTAの液体ホバー塗り（暗色配色時）は `color-mix(in srgb, var(--color-marker) 88%, black)` を使い、いずれも文字とのコントラスト比4.5:1以上を確保します。トークンの値そのものは変更しません。JS からしか読まないトークン（VISION 反転先の `--color-dark-*` など）は **`@theme static`** で定義すること（通常の `@theme` は CSS 未参照の変数を出力しないため、`getComputedStyle` が空になり文字が黒のまま背景に沈む）。
...
| `/privacypolicy` `/transaction` | プライバシーポリシー / 特定商取引法に基づく表記 | 静的（現行サイトの大文字 URL `/PRIVACYPOLICY` `/TRANSACTIONACT` と `/privacy-policy` は _redirects で 301） |
...
| 演出 | 対象 | 実装 |
|---|---|---|
| イントロ幕 | HOME 表示のたび | 参考サイトの手順を再現: SSR の黒幕 → ハイドレーション後にロゴ箱をマーキーのロゴセル `[data-lead] [data-lead-box]` と同じ位置・大きさに置き 0.3s フェード + 12px 上昇で出現（ロゴは動かない）→ 200ms（SP 300ms）後に黒幕（ロゴセル中心の巨大な角丸正方形 `.veil-curtain`）を `transform: scale` でその箱の大きさまで 0.45s で縮める（2026-09-06 に LCP 対策で短縮。以前は 0.5s / 450ms（SP 780ms）/ 0.75s）（黒がロゴに集まる。clip-path はメインスレッド駆動で途中停止するため使わない）→ 終了で `data-boing`・`data-intro` 除去・`kv:launch`、幕は 0.12s フェード。定数は `IntroVeil.tsx` 先頭。reduced-motion / `saveData` / JS 無効ではスキップし `kv:launch` を即発火 |
